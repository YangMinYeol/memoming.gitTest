...
function DaumAddressAPI() { // 다음 우편번호 API
	new daum.Postcode(
	{
	oncomplete : function(data) {                
		...
		if (data.userSelectedType === 'R') {
			...
			document.getElementById('ORDER_DZIPCODE').value = data.zonecode;
			document.getElementById("ORDER_DADD1").value = addr;
                  
			document.getElementById("ORDER_DADD2").focus();
		}
	}).open();
}
...
function fn_chkinfo(){
    ...
    } else if(chk==false) { // 기존 값을 초기화하고 직접 입력
...
<form name="orderWrite" id="orderWrite" method="post" action="/solo/order/orderPay.do">

	<!-- 주문할 상품 리스트 (basketList와 동일) -->
            ...
            <tbody>
				...
				<tr>
					...
					<td><a href="/solo/shop/goodsDetail.do?IDX=${row.GOODS_NUM }">${row.GOODS_NAME }</a>
						<br> 색상: ${row.GOODS_COLOR } <br> 사이즈:${row.GOODS_SIZE } <br>
					</td>
...
<hr>
	...
	<table style="width:100%;">
		...
		<tr>
			<td>&nbsp;&nbsp;</td>
			<td>
          		<input align="right" type="checkbox" name="chkinfo" id="chkinfo" onclick="fn_chkinfo()">주문자 정보와 동일
          	</td>
		</tr>
		<tr>
			<td>받는 사람</td>            
		...
		<tr>
			<td>일반전화</td>
			<td><input type="text" id="ORDER_PHONE2" name="ORDER_PHONE2" class="form-control"  style="width:40%;" placeholder="선택사항"></td>
		</tr>
		<tr>
            <td>휴대전화</td>
		...
		<tr>
            ...
            <td>
				...
				<input type="button" onclick="DaumAddressAPI()" class="btn btn-outline-dark mb-2" value="우편번호 찾기" style="display:inline-block;"><br>
            </td>
		</tr>
		<tr>
			<td>
...
<hr>
	...
	<table style="width:100%;">
		...
		<tr>
			<td>결제 방법</td>
			<td>
			<input class="form-check-input" id="ORDER_PAY" name="ORDER_PAY" type="radio" checked id="radio">
...
<hr>
	...
	<div align="center" style="margin-top:50px;">
		<input type="button" class="btn btn-outline-dark" name="order_pay" value="주문하기" onclick="fn_order_pay()">
		<input type="button" class="btn btn-outline-dark" value="주문취소" onClick="location.href='/solo/shop/basketList.do'">
	</div>
</form>
</div>
