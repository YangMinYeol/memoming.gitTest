	...
	<div style="float:right;">
		...
		<input type="hidden" name="NOTICE_NUM" id="NOTICE_NUM" value="${map.NOTICE_NUM}"/>
			<c:if test="${map.NOTICE_TYPE == 'FAQ' }">
         	<a href="#this" id="modifyFaq"><button type="button" class="btn btn-outline-secondary" style="margin-right:6px;">FAQ수정</button></a>
         	</c:if>
         	<c:if test="${map.NOTICE_TYPE != 'FAQ' }">
         	<a href="#this" id="modify"><button type="button" class="btn btn-outline-secondary" style="margin-right:6px;">공지수정</button></a>
         	</c:if>
			<a href="#this" id="delete"><button type="button" class="btn btn-outline-secondary" style="margin-right:6px;">삭제</button></a>
			<button type="button" class="btn btn-outline-secondary" onclick="javascript:history.go(-1);" style="margin-right:6px;">목록</button>
         </c:if>
         <c:if test="${session_MEMBER.MEM_NICK != map.NOTICE_WRITER}">
         <button type="button" class="btn btn-outline-secondary" onclick="javascript:history.go(-1);" style="margin-right:6px;">목록</button>
         <!--  목록 경로 고쳐야 합니다.(회원) -->
