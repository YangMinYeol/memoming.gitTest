...
$(document).ready(function() {
    ...
    $("#nickCheck").on("click", function(e) { // 닉네임 중복검사
       e.preventDefault();
       fn_nickCheck();
    });
 });
...
if(!$("#MEM_NICK").val()){ // 만약 닉네임 입력을 안하면
       alert("닉네임을 입력하세요"); // alert를 띄우고
       $("#MEM_NICK").focus(); // 닉네임 포커스
       return false;
}

 var MEM_NICK = {MEM_NICK : $('#MEM_NICK').val()}; // 닉네임칸에 있는값을 MEM_NICK변수에 넣음
 ...
 $.ajax({
     ...
     success:function(data){    
         if($.trim(data)=="0"){
            ...
            alert("중복된 닉네임입니다.");
             $('#MEM_NICK').val(''); // 닉네임칸을 공백으로 바꾸고
             $('#MEM_NICK').focus(); // 닉네임칸에 포커스
         }
     },
     error:function(){ // 에러가 발생하면
             alert("닉네임을 입력하세요"); // 닉네임을 입력하세요
     }
 });
};
   ...
   <form id="frm" name="frm" class="form-inline" method="post">
      <table class="joinFormView" style="border:0; cellpadding:1; cellspacing:1;" align="center">
            ...
            <tr>
               ...
               <td>
               <input type="button" name="nickCheck" value="중복확인" class="btn btn-outline-dark"  OnClick="fn_nickCheck()" style="width:100%;">
               </td>
            </tr>
            ...
            <tr>
             ...
             <td>
            <input type="button" onclick="DaumAddressAPI()" class="btn btn-outline-dark" value="우편번호 찾기" style="width:100%;"><br>
            </td>
            </tr>
            <tr>
            <td colspan="2">
         ...
         <tr>
            ...
            <tr>
               ...
               <br>
               <a href="#this" class="btn" id="modify"><input type="button" class="btn btn-outline-dark" value="수 정" style="margin-left:90px;"/></a> 
               <input type="button" value="취 소" class="btn btn-outline-dark" onclick="javascript:window.location='/solo/main/openMainList.do'" />
               <a href="#this" class="btn" id="delete"><input type="button" value="회원탈퇴" class="btn btn-dark"  /></a> 
               </td>
            </tr>
